...
class Mqtt extends Sam
{
    ...
    */
    ...
    public $port = '';
    public $host = '';
    public $cleanstart = false;
    public $virtualConnected = false;
    ...
    */
    public $operations = array("MQTT_CONNECT"     => 1,
        "MQTT_CONNACK"     => 2,
        "MQTT_PUBLISH"     => 3,
        "MQTT_PUBACK"      => 4,
        "MQTT_PUBREC"      => 5,
    ...
    function SAMConnection_MQTT() {
        if ($this->debug) $this->$this->e('SAMConnection_MQTT()');

        if ($this->debug) $this->x('SAMConnection_MQTT()');
    }
    ...
    /* ---------------------------------
       ...
       --------------------------------- */
    function Commit() {
        if ($this->debug) $this->$this->e('SAMConnection_MQTT.Commit()');

        $errNo = 100;
        $error = 'Unsupported operation for MQTT protocol!';
        $rc = false;

        if ($this->debug) $this->x("SAMConnection_MQTT.Commit() rc=$rc");
        return $rc;
    }
    ...
    function Connect($proto, $options=array()) {
        if ($this->debug) $this->e('SAMConnection_MQTT.Connect()');
        ...
        if ($this->debug) $this->x("SAMConnection_MQTT.Connect() rc=$this->virtualConnected");
    ...
    /* ---------------------------------
       ...
       --------------------------------- */
    function Disconnect() {
        if ($this->debug) $this->e('SAMConnection_MQTT.Disconnect()');
        $rc = false;
        ...
        if ($this->debug) $this->x("SAMConnection_MQTT.Disconnect() rc=$rc");
        return $rc;
    }
    ...
    function IsConnected() {
        if ($this->debug) $this->e('SAMConnection_MQTT.IsConnected()');
        $rc = false;
        ...
        if ($this->debug) $this->x("SAMConnection_MQTT.IsConnected() rc=$rc");
        return $rc;
    }
    ...
    /* ---------------------------------
       ...
       --------------------------------- */
    function Peek() {
        if ($this->debug) $this->e('SAMConnection_MQTT.Peek()');

        $errNo = 100;
        $error = 'Unsupported operation for MQTT protocol!';
        $rc = false;

        if ($this->debug) $this->x("SAMConnection_MQTT.Peek() rc=$rc");
        return $rc;
    }
    ...
    /* ---------------------------------
       ...
       --------------------------------- */
    function PeekAll() {
        if ($this->debug) $this->e('SAMConnection_MQTT.PeekAll()');

        $errNo = 100;
        $error = 'Unsupported operation for MQTT protocol!';
        $rc = false;

        if ($this->debug) $this->x("SAMConnection_MQTT.PeekAll() rc=$rc");
        return $rc;
    }
    ...
    function Receive($sub_id, $options=array()) {
        if ($this->debug) $this->e('SAMConnection_MQTT.Receive()');
        $rc = false;
        ...
        if ($this->debug) $this->x("SAMConnection_MQTT.Receive() rc=$rc");
        return $rc;
    }
    ...
    /* ---------------------------------
       ...
       --------------------------------- */
    function Remove() {
        if ($this->debug) $this->e('SAMConnection_MQTT.Remove()');

        $errNo = 100;
        $error = 'Unsupported operation for MQTT protocol!';
        $rc = false;

        if ($this->debug) $this->x("SAMConnection_MQTT.Remove() rc=$rc");
        return $rc;
    }
    ...
    /* ---------------------------------
       ...
       --------------------------------- */
    function Rollback() {
        if ($this->debug) $this->e('SAMConnection_MQTT.Rollback()');

        $errNo = 100;
        $error = 'Unsupported operation for MQTT protocol!';
        $rc = false;

        if ($this->debug) $this->x("SAMConnection_MQTT.Rollback() rc=$rc");
        return $rc;
    }
    ...
    function Send($topic, $message, $options=array()) {
            ...
            if ($hdr) {
                ...
                if ($qos == 1) {
                    ...
                    if ($hdr['mtype'] == $this->operations['MQTT_PUBACK']) {
                        ...
                        if ($len < 2) {
                            if ($this->debug) $this->t("SAMConnection_MQTT.Send() send failed, incorrect length response ($len) received!");
                            $this->errNo = 302;
        ...
        if ($this->debug) $this->x("SAMConnection_MQTT.Send() rc=$rc");
        return $rc;
    }
    ...
    function Subscribe($topic, $options=array()) {
        ...
        $hdr = $this->read_fixed_header($this->sock);
        if (!$hdr) {
            if ($this->debug) $this->t("SAMConnection_MQTT.Subscribe() subscribe failed, no response from broker!");
            $this->errNo = 301;
        ...
        if ($this->debug) $this->x("SAMConnection_MQTT.Subscribe() rc=$rc");
        return $rc;
    }
    ...
    function Unsubscribe($sub_id) {
        ...
        $hdr = $this->read_fixed_header($this->sock);
        if (!$hdr) {
            if ($this->debug) $this->t("SAMConnection_MQTT.Unsubscribe() unsubscribe failed, no response from broker!");
            $this->errNo = 302;
            ...
            if ($hdr['mtype'] == $this->operations['MQTT_UNSUBACK']) {
                ...
                if ($len != 2) {
                    if ($this->debug) $this->t("SAMConnection_MQTT.Unsubscribe() unsubscribe failed, incorrect length response ($len) received!");
                    $this->errNo = 301;
        ...
        if ($this->debug) $this->x("SAMConnection_MQTT.Unsubscribe() rc=$rc");
        return $rc;
    }
    ...
    function remaining_length($l) {
        ...
        if ($this->debug) $this->t("SAMConnection_MQTT.remaining_length() l=$l");

        $rlf = '';
        do {
            $digit = $l % 128;
            $l = ($l - $digit)/128;
            if ($this->debug) $this->t("SAMConnection_MQTT.remaining_length() digit=$digit l=$l");
            ...
            $rlf .= $digit;
            if ($this->debug) $this->t("SAMConnection_MQTT.remaining_length() rlf=$rlf");
        } while ($l > 0);
    ...
    function checkHost($hostname, $port) {
        ...
        if ($this->debug) $this->x("SAMConnection_MQTT.checkHost(rc=$rc)");
        return $rc;
    }
    ...
    function do_connect_now() {
        ...
        if ($this->sub_id == '') {
            ...
            if ($this->debug) $this->t("SAMConnection_MQTT.do_connect_now() sub_id=$this->sub_id");
        } else {
            if ($this->debug) $this->t("SAMConnection_MQTT.do_connect_now() using existing sub_id=$this->sub_id");
        }
        ...
        if (!$this->virtualConnected) {
            ...
            if (!$fp) {
                if ($this->debug) $this->t("SAMConnection_MQTT.do_connect_now() fsockopen failed! ($errNo) $errstr");
                $this->errNo = 208;
        ...
        if ($hdr) {
            if ($hdr['mtype'] == $this->operations['MQTT_CONNACK']) {
                $len = $this->read_remaining_length($this->sock);
                if ($len < 2) {
                    if ($this->debug) $this->t("SAMConnection_MQTT.do_connect_now() connect failed, incorrect length response ($len) received!");
                    $this->errNo = 218;
    ...
    function read_fixed_header($conn) {
        ...
        if (strlen($response) > 0) {
            ...
            $x -= ($dup * 8);
            $mtype = $x / 16;
            if ($this->debug) $this->t("SAMConnection_MQTT.read_fixed_header() mtype=$mtype, dup=$dup, qos=$qos, retain=$ret");
    ...
    function read_topic($conn) {
        if ($this->debug) $this->e('SAMConnection_MQTT.read_topic()');
        $rc = false;
        while (!feof($conn)) {
            $tlen = fread($conn, 2);
        ...
        if ($this->debug) $this->x("SAMConnection_MQTT.read_topic(rc=$rc)");
        return $rc;
    }
